# Agent replay：把一次 invocation 变成可复用 regression fixture

术语遵循 [Agent 文档术语（Terminology）](agent-terminology.md)。

## 目的和边界

V4 已把一次 Agent invocation 保存为 `agent_invocations` 和 ordered `agent_steps`，但 persistence record
本身只是运行证据。第一版 Agent replay 把它转换为可重复检查的 JSON fixture：用相同的
模型输出、工具输入和 persistence 结果来验证工程边界是否因后续改动而退化。
...
- step 的 `ok`、`error`、`observation` 是否仍与完整 `ToolResult contract` 一致。
- invocation `status` 是否仍与 `max_steps_reached` 汇总结果一致。
- `request_user_input` 的输入、choice resolution、单次限制和 pending/completed 状态是否仍一致。
...
hard assertion；native tool calling 也没有 `react_text` parser，所以 parser check 会显示
`not_applicable`。
...
`--overwrite`。replay 命令返回：`0` 表示全部检查通过，`1` 表示 fixture 与当前确定性边界不匹配，
`2` 表示文件、contract 或运行环境错误。
...
4. 在 `test_agent_replay_service.py` 中添加该 fixture 的 replay assertion，并用 fake registry/input
   model 固定工程边界。

仓库的 synthetic fixtures 覆盖 react/native calculator 成功、react parser failure，以及 clarification 的
answered、skip 和 timeout。它们是 regression assets，而不是对真实模型回答质量的排行榜或在线评估平台。
