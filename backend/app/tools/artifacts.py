...
class ToolArtifacts(BaseModel):
    ...
    def validate_outputs(
        ...
        try:
            serialized_chars = serialized_tool_outputs_chars(outputs)
        except (TypeError, ValueError) as exc:
            raise ValueError("tool outputs must be JSON-safe") from exc
        if serialized_chars > MAX_TOOL_OUTPUT_JSON_CHARS:
            raise ValueError(
...
def serialized_tool_outputs_chars(outputs: dict[str, JsonValue]) -> int:
